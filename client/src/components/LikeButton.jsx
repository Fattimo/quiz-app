const LikeButton = (props) => {
    ...
    return (
        <div className="cursor-pointer flex" onClick={props.toggle}>
            {liked ? 
            ...
            {props.tag==="long" ? <div className={`${liked ? "text-red-400" : "text-gray-400"} mx-1 font-semibold text-s tracking-wide uppercase`}>{props.likes} Likes</div> : 
            props.tag==="number" ? <div className={`${liked ? "text-red-400" : "text-gray-400"} mx-1 font-semibold text-s tracking-wide uppercase`}>{props.likes}</div>:""}
        </div>
    )
}
